...
수 없다**. Mathf 쪽은 두 컴파일러 모두에서 정상 측정된다.
...
| **Mathf `SinCos`** | **183.5 M/s** | **2.7e-07, 전 구간 평탄** |
...
> 리뷰 중 이 경로에서 **22% 회귀를 한 번 만들었다가 되돌렸다.** `AbsScalar`를
> 비트 연산(`FromBits(BitsOf(x) & kAbsMask)`)으로 바꿨더니 Slerp가 37.8 → 29.3,
> SinCos가 201 → 184로 떨어졌다. x86에서 `bit_cast`는 값을 범용 레지스터로
...
MSVC, `--benchmark_repetitions=9 --benchmark_enable_random_interleaving=true`,
중앙값. 게이트는 PLAN §4.2의 **DXMath 대비 ±5%**.

| 연산 | Mathf | DirectXMath | 판정 |
|------|-------|-------------|------|
| Add (지연) | 2.19 ns | 2.18 ns | 통과 |
| MulAdd (지연) | 2.13 ns | 2.12 ns | 통과 |
| MulAdd (처리량) | 1.007 G/s | 1.007 G/s | 통과 |
...
| **Cross (지연)** | **15.3 ns** | **13.6 ns** | **미달 −12.5%** |
| Cross (처리량) | 501.9 M/s | 455.2 M/s | +10% |
| mat4 곱 (지연) | 5.27 ns | 5.69 ns | +8% |
| mat4 곱 (처리량) | 341.3 M/s | 329.4 M/s | 통과 |
| mat4 역행렬 | 74.1 M/s | 68.3 M/s | +9% |
| **mat4 전치** | **186.4 M/s** | **501.9 M/s** | **미달 −63%** |
| 배치 정점 변환 | 851.0 M/s | 652.4 M/s | **+30%** |
| 쿼터니언 곱 (지연) | 5.18 ns | 4.46 ns※ | 미달 −16% |
| 쿼터니언 곱 (처리량) | 598.8 M/s | 591.9 M/s | 통과 |
| Slerp | 34.1 M/s | 30.6 M/s | +11% |
| 벡터 회전 | 203.9 M/s | 178.4 M/s | +14% |
| 쿼터니언 → 행렬 | 229.4 M/s | 160.6 M/s | +43% |
| TRS 합성 | 194.6 M/s | 133.8 M/s | +45% |
| SinCos | 194.6 M/s | 259.5 M/s | −25% (의도) |

※ 패킹 타입과의 공정 비교값. 레지스터 상주 `XMVECTOR`는 각각 2.13 / 4.00 ns다.

### 감사 자체가 찾아낸 것: 게이트 항목 세 개에 벤치가 없었다

PLAN §4.2가 게이트 대상으로 명시한 `Cross`, `Transpose(Mat4)`, 배치 정점 변환은
**단 한 번도 측정된 적이 없었다.** 평가할 수 없는 게이트는 게이트가 아니다.
셋을 추가하자 둘이 미달로 드러났다 — 즉 이 감사가 없었으면 "전 항목 통과"라고
적을 뻔했다. 배치 정점 변환은 반대로 DXMath의 전용 스트리밍 진입점(
`XMVector3TransformCoordStream`)보다 30% 빠르다. 전용 API 없이 평범한 루프로
두기로 한 설계가 값을 한 셈이다.

### mat4 전치 −63%: 원인은 특정, 수정은 미완

어셈블리를 보면 연산 자체는 최적이다 — 적재 4, 셔플 8, 저장 4. 낭비는 그
주변에 있다:

1. `Matrix4x4 result;`의 **기본 생성자가 64바이트를 0으로 채우고 즉시 덮어쓴다**
   (`vmovups YMMWORD PTR [rcx], ymm0` 두 번). 곱셈과 역행렬에서는 컴파일러가
   이 저장을 제거했는데 전치에서는 남았다.
2. `out[i] = Transpose(in[i])`는 임시 객체를 만든 뒤 **64바이트를 복사**한다.
   DXMath의 `XMStoreFloat4x4(&out[i], ...)`는 목적지에 곧바로 쓴다.

둘 다 값 반환 API의 형태에서 나오는 비용이지, 전치 알고리즘의 문제가 아니다.
고치려면 0 채움을 피하는 내부 생성 경로나 `TransposeTo(out, in)` 형태가 필요한데,
Phase 4에서 "더 올바른" 형태로 바꿨다가 22% 회귀를 만든 전례가 있어 측정 없이
손대지 않았다. **미해결로 기록한다.**

`Cross` 지연 −12.5%도 미해결이다. 처리량은 오히려 10% 앞서므로 배치 작업에는
영향이 없고, 직렬 의존 사슬에서만 나타난다.
