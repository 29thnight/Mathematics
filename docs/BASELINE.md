...
### fixed_ranges — 파이프 closure는 공짜, MSVC의 `std::views`는 아니다
...
예외가 하나 있고, 이건 진짜 코드젠 차이다. **MSVC는 `std::views` 경로를 언롤하지
못한다.**

| | MSVC direct | MSVC `std::views` |
|---|---|---|
| 성분 sum | 1.190 ns | 2.825 ns |
| 성분 transform | 1.186 ns | 3.357 ns |
| 행 sum | 3.662 ns | 7.629 ns |

`direct_sum`은 `vmovss` + `vaddss` 3개의 직선 코드인데, `view_sum`은 `rax`가 0..3을
도는 **실제 내부 루프**를 만들고 그 안에서 매 반복 결과를 스택에 저장한다. 성분 수가
4로 고정인데도 언롤하지 못한 것이다. 벤치마크 바깥 루프 외에 역방향 분기가 몇 개
남았는지로 세 항목 모두 확인된다 — `view_sum` 2개, `view_transform` 2개(안쪽에
`vaddss` + `vmovss [rax]`), `rows_view_sum` 3개인 반면 `direct`·`fold`·`fixed`·
`pipeline` 형태는 전부 1개다.

clang은 같은 소스에서 이 문제가 없다(성분 sum 0.366 대 0.484 ns로 오히려 뷰가 빠르다).
**§8이 "네 transform이 같은 루프 본문으로 컴파일된다"고 적은 표는 clang 수치이고,
거기서는 실제로 네 형태가 같다.** 두 절이 어긋나는 것이 아니라, 언롤 실패가
MSVC에만 있다.

> **단, 이 계열의 절대값은 인용하지 말 것.** §8이 기록한 대로 `.text`를 16바이트만
> 밀어도 2배 페널티가 벤치들 사이를 옮겨 다닌다. 위 표를 신뢰하는 근거는 수치가
> 아니라 **어셈블리에서 확인한 구조 차이**(추가 루프와 반복당 저장)이며, 그것만이
> 재링크를 견딘다. 형태 사이에 구조 차이가 없는 조합에서는 수치 차이도 읽지 않는다.

### GUIDE의 두 문장은 이제 측정으로 뒷받침된다

GUIDE.md는 "파이프 closure 자체는 어셈블리에서 직접 호출과 동일하게 제거된다"고
쓰고, 바로 다음 문단에서 "range-for는 정상적인 iterator 루프이므로 MSVC에서 자동
전개가 보장되지는 않는다"고 덧붙인다. 위 측정이 두 문장을 각각 확인한다 —
전자는 세 쌍의 바이트 동일성으로, 후자는 MSVC `std::views` 경로의 2.4~2.8배
페널티로. 그 경고가 왜 필요한지를 보여주는 수치가 이제 있다.

주의할 것은 두 문장이 서로 다른 대상을 가리킨다는 점이다. 공짜인 것은 **라이브러리
자신의 `*_fixed` 종단을 파이프로 붙이는 형태**이고, 언롤에 실패하는 것은 **표준
`std::views` 조합**이다. 고정 extent가 중요한 자리에서는 `fold_fixed` 계열을 쓰면
