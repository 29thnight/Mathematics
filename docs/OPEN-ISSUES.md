...
> Zen 3에서 15.12%, Cascade Lake에서 6.44%로 내려왔다. 다만 15.12%는 §10이 열어 둔
> 잔여 격차(19.44의 스택 왕복)를 그대로 안고 있는 값이라, 좁히더라도 5%로는 못 간다.
> 러너별 수치가 몇 번 더 쌓이면 20% 언저리가 근거 있는 자리다. 나머지 네 줄의 근거는
> 그대로다 — 배치 편차는 손대지 않았다.
