...
> [BASELINE §10](BASELINE.md)이 닫은 결함이 섞인 수치다. 결함을 걷어낸 뒤 관측된 최대는
> Zen 3의 15.12%다. 다만 그 값은 §10이 열어 둔 잔여 격차(19.44의 스택 왕복)를 그대로 안고
> 있으므로 좁히더라도 5%로는 못 간다. 표본은 러너당 한 쌍뿐이고, 같은 Cascade Lake에서
> 6.44%와 10.26%가 함께 나왔다 — 이 줄을 좁히려면 **이 절이 기록한 배치 편차 때문에
> 표본이 더 필요하다.** 나머지 네 줄의 근거는 그대로다.
