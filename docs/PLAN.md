...
- [ ] **쿼터니언 곱 지연만 미달**: 5.24ns 대 4.51 (패킹 형태 DXMath 기준) — 16% 열세.
      어셈블리는 곱 1회당 10개 명령·스필 없음·상수 호이스팅 완료로 DXMath와 동형이고,
      남은 차이는 명령 수로 설명되지 않는다. 원인 미규명
- [ ] `SinCos`는 DXMath의 `XMScalarSinCos`보다 27% 느리다(184 대 252 M/s). 범위 축소를
...
- **실제 버그: `ATan2(±inf, ±inf)`가 NaN이었다.** `inf/inf`는 IEEE-754에서 NaN이라
  ...
  분기로 수정. 헤더가 "C 라이브러리와 같이 정의된다"고 약속하고 있었으므로 계약 위반.
- **정책 불일치: `up`이 `forward`와 평행할 때 뷰 행렬이 조용히 망가졌다.** 외적이
  ...
  행렬의 `Inverse`, 0 벡터의 `Normalize`)에 맞춰 항등행렬을 반환하도록 수정.
- **부호 있는 0이 `ASin`/`ATan`을 통과하며 소실됐다.** 작은 인자 조기 반환으로 수정 —
  이 반환은 부호뿐 아니라 **상대 정확도**도 고친다. 1e-6 근처에서 `kHalfPi - ACosCore(x)`는
  두 큰 값의 상쇄로 절대 2e-7, 즉 **상대 25%** 오차를 냈고, 작은 회전의
  `ToAxisAngle` 각도를 잡음으로 만들고 있었다.
- **제가 만든 회귀 하나.** 위 부호 문제를 `AbsScalar`의 비트 연산화로 고치려다
  Slerp를 22%, SinCos를 8% 느리게 만들었다 — `bit_cast`가 x86에서 범용 레지스터를
  왕복하는데, 이 함수는 `Reducible()`을 통해 모든 삼각함수 호출 경로에 있다.
  되돌리고 조기 반환으로 해결. **벤치 없이 "더 올바른" 형태로 바꾸면 이렇게 된다.**
- **리뷰어가 손 유도로 무결 판정한 것**: SIMD 해밀턴 곱 16항, `ToEuler` 유도와
...
- **테스트 196 → 203.** 반사와 비자명 회전의 조합(Phase 3의 identity×identity 구멍과
  같은 모양), `Slerp`/`Nlerp`/`ToEuler`/`ToAxisAngle`/`Decompose`/`LookTo*`/`Orthographic*`의
  constexpr 증명, `Tan`의 문서화된 경계까지의 정확도, 작은 각도 `ToAxisAngle`.
...
5. **측정은 교차 실행으로 한다.** 이 기계는 부하가 흔들려 같은 벤치가 실행 간 20%까지
   움직였다. 교차하지 않으면 그 드리프트가 라이브러리 간 차이로 둔갑한다.
...
1. **문서의 예제는 테스트다.** 컴파일되지 않는 문서는 없느니만 못하다 — 자신 있게
   ...
   실행 검증으로 옮겼다.
2. **감사는 "측정치가 좋은가"가 아니라 "측정하고 있기는 한가"부터 묻는다.**
...
4. **탐침도 코드다.** DXMath 규약을 읽어낸 첫 탐침이 `printf` 인자에서
   ...
   거리 0"이라는 **틀린 결론**을 냈다. 관례를 관측할 때 관측 장치부터 의심해야
   한다.
