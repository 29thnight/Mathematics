...
> **이 규약은 IEEE 특수값을 지키는 부동소수점 모델에서만 성립한다.** `/fp:precise`나
> GCC·Clang의 기본 설정이 그렇다. fast 계열 모델은 무한대와 NaN을 정확히 다루지 않아도
> 되므로 아래 표의 무한·NaN 행은 보장되지 않는다. 특히 Clang·GCC의 `-ffast-math`(clang-cl의
> `/fp:fast` 포함)는 `-ffinite-math-only`로 무한대·NaN이 없다고 가정하고, 그 검사들을
> 실제로 컴파일에서 지운다 — Mathematics의 `atan2`·`normalize`에서 확인했다. 함수는 남고
> 경계 처리만 사라진다. 비유한 입력이 들어올 수 있는 빌드에서 fast 모델을 쓴다면
> `-fno-finite-math-only`를 함께 준다. 0 벡터·특이 행렬 같은 유한한 퇴화 입력은 이
> 가정의 대상이 아니지만, fast 모델의 다른 완화(연산 재결합 등)가 행렬식처럼 계산된 값을
> 정확한 0에서 비켜 놓을 수는 있다. 그쪽은 확인하지 않았다.
