...
> 이 결과는 저장 타입(`float2/3/4`)과 레지스터 타입(`vec_reg`)을 분리하는
> PLAN.md §2.2의 3계층 설계가 왜 필요한지도 함께 증명한다. `float4`는 메모리
> 레이아웃 용도로만 쓰고, 연산은 반드시 `vec_reg`로 승격시켜야 한다.
...
Phase 1 벤치마크에서 Mathematics가 DXMath보다 **1.9배 느린** 것이 관측되었다.
추적 결과 원인은 백엔드 구현이 아니라 **`MATHEMATICS_IF_CONSTEVAL`의 C++20 대체 구현**이었다.
...
이분 탐색으로 범인은 **루프 뒤의 드리프트 가드 한 줄**(`get_x(acc)` 비교)로 좁혀졌고,
